...
@Component({
  selector: 'app-management-monitored-assessment',
  templateUrl: './management-monitored-assessment.component.html',
  styleUrls: ['./management-monitored-assessment.component.scss']
})
export class ManagementMonitoredAssessmentComponent {
...
//   key: 'suggestions_id',
//   load: () => this.sendRequest(URL + '/MitigationController/GetSuggestionListById/'+this.mitigations_id),

//   insert: (values) => this.sendRequest(URL + '/MitigationController/insertManagementSuggestionsList', 'POST', {
//       //values: JSON.stringify(values)
//       values
//   }),
//   update: (key, values) => this.sendRequest(URL + '/MitigationController/UpdateSuggestionManagement', 'PUT', {
//        key,
//        //values: JSON.stringify(values)
//       values
//    }),
...
this.SuggestionsInfo.mitigations_id= Number(this.mitigations_id);

this.SuggestionsInfo.suggested_by=this.UserIdValue;
...
sendformanagement(){
  ...
  this.http.put(URL+'/MitigationController/UpdateMitigationStatus/'+this.mitigations_id,{headers}).subscribe((data:any)=>{

    console.log('API Response:', data);

    const message = "Submited Suggestions Successfully";
    const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
      width: '550px',
      data: { message: message},
...
secondgrid(e:any){
 ...
 exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List Of  Assessment Suggestions ']);
      worksheet.addRow([]);
      exportDataGrid({
        worksheet: worksheet,
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) {
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListOfAssessmentSuggestions.xlsx");
        });
      });
      e.cancel = true;
    }
      ...
      const doc = new jsPDF();
      doc.text("List of  Assessment Suggestions", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('ListOfAssessmentSuggestions.pdf');
      });
    }
