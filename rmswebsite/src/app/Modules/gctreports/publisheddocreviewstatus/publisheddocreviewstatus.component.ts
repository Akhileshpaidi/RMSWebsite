...
@Component({
  selector: 'app-publisheddocreviewstatus',
  templateUrl: './publisheddocreviewstatus.component.html',
  styleUrls: ['./publisheddocreviewstatus.component.scss']
})
export class PublisheddocreviewstatusComponent {
...
DateSearchFilter() {
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsController/GetDocumentReviewStatus', formData)
                .then(response => {
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List of Published Documents Review Status']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Published Documents Review Status.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Published Documents Review Status", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('List of Published Documents Review Status.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Published Documents Review Status.csv");
      });
    }); 
    e.cancel = true;
  } 
