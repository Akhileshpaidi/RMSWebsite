...
@Component({
  selector: 'app-reactivate-questions',
  templateUrl: './reactivate-questions.component.html',
  styleUrls: ['./reactivate-questions.component.scss']
})
export class ReactivateQuestionsComponent  {
  treeBoxValue: string;
  gridDataSource: any;
  userdata:any;
  gridBoxValue:any;
        ...
        store: new CustomStore({
          ...
          load: () => {
            return new Promise((resolve, reject) => {
              this.httpClient.get(URL + '/QuestionBank/GetInActiveQuestionsAll/' + this.userid, { headers })
                .subscribe(
                  (res: any) => resolve(res.data || res), // Adjust `res.data` based on your response structure
                  (err) => {
  ...
  constructor(private httpClient: HttpClient, private ref: ChangeDetectorRef,
    private formBuilder: FormBuilder,private session: SessionService,public dialog: MatDialog) {
    ...
    if (this.form.valid ) {
      ...
      const dialogRefConfirmation = this.dialog.open(ToasterComponent, {
        ...
        data: {
          ...
          message: 'Are you sure you want to Reactivate question?',
        },
      });
      ...
      dialogRefConfirmation.afterClosed().subscribe((result: boolean) => {
        ...
        this.httpClient.put(URL + '/QuestionBank/ReActivateQuestionByID', formData)
            .subscribe(
                (response: any) => {
                  const message = "Question Reactivated successfully.";
