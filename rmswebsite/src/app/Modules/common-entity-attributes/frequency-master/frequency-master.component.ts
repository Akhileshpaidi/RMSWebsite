...
export class FrequencyMasterComponent {
  dataSource2: any;
  userdata: any;
  typedata: any;
 ...
 SubTypeinfo: frequencymaster= new frequencymaster();
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    ...
    this.dataSource2 = new CustomStore({
      ...
      load: () => this.sendRequest1(URL + '/frequencymaster/GetfrequencymasterDetails'),
      
      insert: (values) => this.sendRequest1(URL + '/frequencymaster/InsertfrequencymasterDetails', 'POST', {
          // values: JSON.stringify(values)
          values
      }),
      update: (key, values) => this.sendRequest1(URL + '/frequencymaster/UpdatefrequencymasterDetails', 'PUT', {
           key,
          //  values: JSON.stringify(values)
          values
       }),
       remove: (key) => this.sendRequest1(URL + '/frequencymaster/DeletefrequencymasterDetails', 'DELETE', {
           key
       })
...
if (e.format === 'xlsx') {
  ...
  //alert("test")
  const doc = new jsPDF();
  doc.text('Frequency Master',75,10);
  doc.setFontSize(12);
  exportDataGridToPdf({
    jsPDFDocument: doc,
    component: e.component,
