...
@Component({
  selector: 'app-risk-admin-risk-intensity',
  templateUrl: './risk-admin-risk-intensity.component.html',
  styleUrls: ['./risk-admin-risk-intensity.component.scss']
})
export class RiskAdminRiskIntensityComponent {
  dataSource: any;
  start:any=1;
  userdata:any;
  inputAttr:any;
  ...
  max:number=0;
  valArray:number[][]=[];
  riskCatlev:risk_admin_riskintensity=new risk_admin_riskintensity();

  constructor(private http: HttpClient, private session: SessionService,) {
    this.dataSource = new CustomStore({
        key: 'risk_admin_riskIntensityid',
        load: () => this.sendRequest(URL + '/risk_admin_riskintensity/Getrisk_admin_riskintensity'),
        
        insert: (values) => this.sendRequest(URL + '/risk_admin_riskintensity/Insertrisk_admin_riskintensity', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/risk_admin_riskintensity/Updaterisk_admin_riskintensity', 'PUT', {
             key,values
         }),
         remove: (key) => this.sendRequest(URL + '/risk_admin_riskintensity/Deleterisk_admin_riskintensity', 'DELETE', {
             key
         })
    });
...
onValueChanged(evt: any, data: any): void {  
 ...
 // alert(JSON.stringify(data))
    this.riskCatlev.risk_admin_riskIntensityid=0;
    
     this.params(data);
   }
   
   updateParameters(data:any={}){
   this.riskCatlev.risk_admin_riskIntensityid=data.key;
   ...
   // this.riskCatlev.risk_natureof_cont_occu_id=data.values.risk_natureof_cont_occu_id;
    this.riskCatlev.risk_admin_riskIntensityname=data.values.risk_admin_riskIntensityname;
    this.riskCatlev.risk_admin_riskIntensitydesc=data.values.risk_admin_riskIntensitydesc;
