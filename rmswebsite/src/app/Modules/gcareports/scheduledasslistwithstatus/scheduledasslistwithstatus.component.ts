...
@Component({
  selector: 'app-scheduledasslistwithstatus',
  templateUrl: './scheduledasslistwithstatus.component.html',
  styleUrls: ['./scheduledasslistwithstatus.component.scss']
})
export class ScheduledasslistwithstatusComponent {
  ...
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsController/getAssScheduledListwithStatus', formData)
                .then(response => {
...
this.DateSearchFilterforAssessorsList(uq_ass_schid,verson_no);
...
this.dataSource = new CustomStore({
    ...
    load: () => {
        ...
        return this.sendRequest1(URL + '/GovControlReportsController/getAssScheduledListmappedusers/'+id+"/"+verson_no+"/",formData)
            .then(response => {
                console.log("Request successful, response: ", response);
                return response;
            })
...
this.dataSource2 = new CustomStore({
  ...
  load: () => {
      ...
      return this.sendRequest1(URL + '/GovControlReportsController/getexempteduserlist/'+id,formData)
          .then(response => {
              console.log("Request successful, response: ", response);
              return response;
          })
...
this.DateSearchFilter();
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List of Scheduled Assessment Status']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListofScheduledAssessmentStatus.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Scheduled Assessment Status", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('ListofScheduledAssessmentStatus.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListofScheduledAssessmentStatus.csv");
      });
    }); 
    e.cancel = true;
  } 
