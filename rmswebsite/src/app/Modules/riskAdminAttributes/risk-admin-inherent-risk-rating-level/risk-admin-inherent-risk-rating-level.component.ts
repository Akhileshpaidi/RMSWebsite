...
import { risk_admin_inherriskratinglevl, risk_admin_riskappetite, risk_admin_riskintensity, risk_admin_risktolerance, risk_intensity, risk_natureof_cont_occu, riskImpactRating, RiskInherentRatingLevel, RiskLikelihood} from '../../../inspectionservices.service';
...
@Component({
  selector: 'app-risk-admin-inherent-risk-rating-level',
  templateUrl: './risk-admin-inherent-risk-rating-level.component.html',
  styleUrls: ['./risk-admin-inherent-risk-rating-level.component.scss']
})
export class RiskAdminInherentRiskRatingLevelComponent {
  dataSource: any;
  start:any=1;
  inputAttr:any;
  value:any;
  ...
  max:number=0;
  valArray:number[][]=[];
  riskCatlev:risk_admin_inherriskratinglevl=new risk_admin_inherriskratinglevl();

  constructor(private http: HttpClient, private session: SessionService,) {
    this.dataSource = new CustomStore({
        key: 'risk_admin_inherRiskRatingLevlid',
        load: () => this.sendRequest(URL + '/risk_admin_inherriskratinglevl/Getrisk_admin_inherriskratinglevl'),
        
        insert: (values) => this.sendRequest(URL + '/risk_admin_inherriskratinglevl/Insertrisk_admin_inherriskratinglevl', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/risk_admin_inherriskratinglevl/Updaterisk_admin_inherriskratinglevl', 'PUT', {
             key,values
         }),
         remove: (key) => this.sendRequest(URL + '/risk_admin_inherriskratinglevl/Deleterisk_admin_inherriskratinglevl', 'DELETE', {
             key
         })
    });
...
onValueChanged(evt: any, data: any): void {  
 ...
 // alert(JSON.stringify(data))
    this.riskCatlev.risk_admin_inherRiskRatingLevlid=0;
    
     this.params(data);
   }
   
   updateParameters(data:any={}){
   this.riskCatlev.risk_admin_inherRiskRatingLevlid=data.key;
   ...
   // this.riskCatlev.risk_natureof_cont_occu_id=data.values.risk_natureof_cont_occu_id;
    this.riskCatlev.risk_admin_inherRiskRatingLevlname=data.values.risk_admin_inherRiskRatingLevlname;
    this.riskCatlev.risk_admin_inherRiskRatingLevlDesc=data.values.risk_admin_inherRiskRatingLevlDesc;
