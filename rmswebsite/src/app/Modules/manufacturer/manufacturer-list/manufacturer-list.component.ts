...
export class ManufacturerListComponent {
  ...
  checked: any;
  sessionData: any;
  displayedColumns = ['srno', 'name', 'description', 'edit', 'status', 'remove'];
  ...
  editManufacturer(element: any) {
    this.router.navigate(['dashboard/manufacturer/create-update-manufacturer'], {
      state: { data: element },
    });
  }
  ...
  getManufacturerList() {
    ...
    this.manufacturer.CRUDManufacturer(encryptedPayload).subscribe((response: any) => {
      ...
      if (response.ResponseCode === '0') {
        let manufacturerList = this.encrypt.decryptionAES(response.ResponseData);
        let data = JSON.parse(manufacturerList);
        this.dataSource = new MatTableDataSource(data.manufacturer);
        this.dataSource.paginator = this.paginator;
