...
@Component({
  selector: 'app-compliance-risk-classification-criteria',
  templateUrl: './compliance-risk-classification-criteria.component.html',
  styleUrls: ['./compliance-risk-classification-criteria.component.scss']
})
export class ComplianceRiskClassificationCriteriaComponent {
  dataSource: any;
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    this.dataSource = new CustomStore({
        ...
        load: () => this.sendRequest(URL + '/Complianceriskcriteria/GetComplianceriskcriteriaDetails'),
        
        insert: (values) => this.sendRequest(URL + '/Complianceriskcriteria/InsertComplianceriskcriteriaDetails', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/Complianceriskcriteria/UpdateComplianceriskcriteriaDetails', 'PUT', {
             key,
            //  values: JSON.stringify(values)
            values
         }),
         remove: (key) => this.sendRequest(URL + '/Complianceriskcriteria/DeleteComplianceriskcriteriaDetails', 'DELETE', {
             key
         })
    });
