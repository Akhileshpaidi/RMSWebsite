...
@Component({
  selector: 'app-jurisdiction-location-list',
  templateUrl: './jurisdiction-location-list.component.html',
  styleUrls: ['./jurisdiction-location-list.component.scss']
})

export class jurisdictionlocationlist implements OnInit {
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    this.dataSource = new CustomStore({
      ...
      load: () => this.sendRequest(URL + '/JurisdictionLocation/GetJurisdictionLocationDetails'),
      
      insert: (values) => this.sendRequest(URL + '/JurisdictionLocation/InsertJurisdictionLocationDetails', 'POST', {
           //values: JSON.stringify(values)
          values
      }),
      update: (key, values) => this.sendRequest(URL + '/JurisdictionLocation/UpdateJurisdictionLocationDetails', 'PUT', {
           key,
          //  values: JSON.stringify(values)
          values
       }),
       remove: (key) => this.sendRequest(URL + '/JurisdictionLocation/DeleteJurisdictionLocationDetails', 'DELETE', {
           key
       })
  });
...
//   private fetchStatesByCountry(countryId: any) {
//     alert("country id"+ this.selectedCountryId)
//     // Replace 'YOUR_BACKEND_API_URL/states' with the actual endpoint to fetch states by country from your backend
// this.http.get<any[]>(`/States/GetStates?countryId=${countryId}`).subscribe(
//     //this.http.get<any[]>(`/States/GetStates?countryId=${countryId}`).subscribe(
//       (states: any[]) => {
//         this.StateMaster = states;
//       },
//       error => {
