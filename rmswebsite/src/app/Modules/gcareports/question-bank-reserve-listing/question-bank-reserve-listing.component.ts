...
@Component({
  selector: 'app-question-bank-reserve-listing',
  templateUrl: './question-bank-reserve-listing.component.html',
  styleUrls: ['./question-bank-reserve-listing.component.scss']
})
export class QuestionBankReserveListingComponent {
  ...
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsContoller/GetQuestionBank', formData)
                .then(response => {
...
this.DateSearchFilterforAssessorsList(selectedRowData.user);
...
this.dataSource = new CustomStore({
    ...
    load: () => {
        ...
        return this.sendRequest1(URL + '/GovControlReportsContoller/userquestionsList/'+id,formData)
            .then(response => {
                console.log("Request successful, response: ", response);
                return response;
            })
...
this.dataSource2 = new CustomStore({
  ...
  load: () => {
      ...
      return this.sendRequest1(URL + '/GovControlReportsContoller/userquestionsListforInactive/'+id,formData)
          .then(response => {
              console.log("Request successful, response: ", response);
              return response;
          })
 ...
 addMinutes = (rowData: { estimated_time: string; }) => {
  return rowData.estimated_time + ' Minutes';
};
...
this.DateSearchFilter();
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['Question Bank Reserve Listing']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "QuestionBankReserveListing.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("Question Bank Reserve Listing", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('QuestionBankReserveListing.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "QuestionBankReserveListing.csv");
      });
    }); 
    e.cancel = true;
  } 
