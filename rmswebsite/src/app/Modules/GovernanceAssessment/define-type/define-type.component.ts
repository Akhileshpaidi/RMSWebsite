...
@Component({
  selector: 'app-define-type',
  templateUrl: './define-type.component.html',
  styleUrls: ['./define-type.component.scss']
})
export class DefineTypeComponent {
 ...
 SubTypeinfo: SubType= new SubType();
  ...
  constructor(private http: HttpClient) {
    this.dataSource = new CustomStore({
        ...
        load: () => this.sendRequest(URL + '/Type/GetTypeDetails'),
        
        insert: (values) => this.sendRequest(URL + '/Type/InsertTypeDetails', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/Type/UpdateTypeDetails', 'PUT', {
             key,
            //  values: JSON.stringify(values)
            values
         }),
         remove: (key) => this.sendRequest(URL + '/Type/DeleteTypeDetails', 'DELETE', {
             key
         })
...
insertParameters(data:any={}){
 ...
 //this.Typeinfo.type_id=0;
 this.params(data);
}

updateParameters(data:any={}){
this.Typeinfo.type_id=data.key;
 this.params(data);
}

params(data:any={}){
 
  this.Typeinfo.type_Name=data.values.type_Name;
 this.Typeinfo.type_DESC=data.values.type_DESC;
...
exportGrid(e:any) {
  if (e.format === 'xlsx') {
    ...
    worksheet.addRow(['Type of Assessment Template Type']);
    worksheet.addRow([]); 
    exportDataGrid({ 
      worksheet: worksheet, 
      ...
      workbook.xlsx.writeBuffer().then(function(buffer) { 
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "AssessmentTemplateType.xlsx"); 
      }); 
    }); 
    e.cancel = true;
  } 
    ...
    const doc = new jsPDF();
    doc.text('Type of Assessment Template Type ',75,10);
    doc.setFontSize(12);
    ...
    exportDataGridToPdf({
      ...
      doc.save('AssessmentTemplateType.pdf');
    });
  }
}
...
//   }).then(function() {
//     workbook.xlsx.writeBuffer().then(function(buffer) { 
//       saveAs(new Blob([buffer], { type: "application/octet-stream" }), "SubTypeAssessment.xlsx"); 
//     }); 
//   }); 
//   e.cancel = true;
// } 
...
//     component: e.component,
//   }).then(() => {
//     doc.save('SubTypeAssessment.pdf');
//   });
// }
// }
