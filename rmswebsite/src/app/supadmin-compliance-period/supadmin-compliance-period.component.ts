...
@Component({
  selector: 'app-supadmin-compliance-period',
  templateUrl: './supadmin-compliance-period.component.html',
  styleUrls: ['./supadmin-compliance-period.component.scss']
})
export class SupadminCompliancePeriodComponent {
  ...
  constructor(private http: HttpClient, private session: SessionService,  private zone: NgZone) {
    this.CompliancePerioddataSource = new CustomStore({
        ...
        load: (options) => {
          return this.sendRequest(URL + '/SupAdmin_complianceperiod/GetcomplianceperiodDetails')
        ...
        insert: (values) => this.sendRequest(URL + '/SupAdmin_complianceperiod/InsertcomplianceperiodDetails', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/SupAdmin_complianceperiod/UpdatecomplianceperiodDetails', 'PUT', {
             key,
            //  values: JSON.stringify(values)
            values
         }),
         remove: (key) => this.sendRequest(URL + '/SupAdmin_complianceperiod/DeletecomplianceperiodDetails', 'DELETE', {
             key
         })
    });
