...
@Component({
  selector: 'app-my-question-bank-listing',
  templateUrl: './my-question-bank-listing.component.html',
  styleUrls: ['./my-question-bank-listing.component.scss']
})
export class MyQuestionBankListingComponent {
  ...
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsContoller/userquestionsList/'+this.Useridvalue, formData)
                .then(response => {
 ...
 addMinutes = (rowData: { estimated_time: string; }) => {
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['My Question Bank Listing']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "MyQuestionBanklisting.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("My Question Bank Listing", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('MyQuestionBanklisting.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "MyQuestionBanklisting.csv");
      });
    }); 
    e.cancel = true;
  } 
