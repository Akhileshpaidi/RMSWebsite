...
@Component({
  selector: 'app-acknowledge-monitored-assess-mitigation',
  templateUrl: './acknowledge-monitored-assess-mitigation.component.html',
  styleUrls: ['./acknowledge-monitored-assess-mitigation.component.scss']
})
export class AcknowledgeMonitoredAssessMitigationComponent {
  @ViewChild('stepper')
  stepper!: MatStepper;
...
tentativeDateValidator() {
  return (control:any) => {
    const tentativeDate = control.value;
    const endDate = this.taskForm.get('EndDate')?.value;

    if (tentativeDate && endDate && tentativeDate < endDate) {
      return { invalidTentativeDate: true };
    }
...
makeAsyncDataSource(http:HttpClient) {
  ...
  return new CustomStore({
    ...
    key: 'mitigations_id',
    load() {
      return lastValueFrom(http.get(`${URL}/MitigationController/GetMitigatedScheduleAssesment`, { headers }));
    },
  });
}
...
localStorage.getItem('mitigations_id');
...
UpdateAction(){
  let formData:any;
  let acknowledge = this.taskForm.value.Acknowledge ? 1 : 0; // Convert boolean to number
  // Determine the values of dependent fields based on conditions
  let actionRequired = acknowledge === 1 ? this.taskForm.value.ActionRequired : null;
  let comments = acknowledge === 1 ? this.taskForm.value.Comments : null;
  let manInputReq = (acknowledge === 1 && actionRequired === 1) ? this.taskForm.value.ManInputReq : null;
  let endDate = (acknowledge === 1 && actionRequired === 1 && manInputReq === 1) ?this.convertISOdate(this.taskForm.value.EndDate) : null;
  let selectPo = (acknowledge === 1 && actionRequired === 1) ? this.taskForm.value.SelectPo : null;
  let tentativeDate = (acknowledge === 1 && actionRequired === 1 && selectPo != null) ? this.convertISOdate(this.taskForm.value.Tentativedate) : null;
  let suggestiveDoc = (acknowledge === 1 && actionRequired === 1) ? this.taskForm.value.SuggestiveDoc : null;
  ...
  formData = {
    ...
    notify_management: manInputReq,
    input_date: endDate,
    assign_responsibility: selectPo,
    tentative_timeline: tentativeDate,
    suggested_documents: suggestiveDoc,
    acknowledge_by: this.UserIdValue,
  };
if(actionRequired===1){
  if(manInputReq===1 && endDate===null){
    ...
    if(selectPo===1&&tentativeDate===null){
      const message="Please Select Tentative End Date";
      const dialogErrorRef=this.dialog.open(DaidailogeComponent,{
      data:{
      message:message},
      width:'550px',
      ...
      if(this.taskForm.valid){
        ...
        this.http.put(URL + '/MitigationController/UpdateSuggestionsList',formData,{headers})
        .subscribe((response: any) => {
      ...
      this.secondgrid(this.mitigations_id);
        ...
        const message = "Please Enter valid values.";
          const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
            width: '550px',
            data: { message: message},
...
getTempId(e:any){
  ...
  this.http.get(URL+'/MitigationController/GetTemplateIdmitigationid/'+id, {headers})
...
sendformanagement(){
  ...
  this.http.post(URL+'/MitigationController/UpdateActionStatus/'+this.mitigations_id,{headers}).subscribe((data:any)=>{
    ...
    const message = "Mitigation Tasks Acknowledged Successfully";
    const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
      width: '550px',
      data: { message: message},
...
unacknowledgegrid(e:any){
  ...
  this.dataSource = new CustomStore({
    ...
    key: 'suggestions_id',
    load: () => this.sendRequest(URL + '/MitigationController/UnAckGetSuggestionsList/'+id),
...
this.taskForm.get('Acknowledge')?.setValue(this.selectedRowData.acknowledge);
...
this.taskForm.get('ManInputReq')?.setValue(this.selectedRowData.notify_management);
...
this.taskForm.get('Tentativedate')?.setValidators([Validators.required, this.tentativeDateValidator()]);
...
convertISOdate(date:Date):Date{
 // this.taskForm.value.Tentativedate
