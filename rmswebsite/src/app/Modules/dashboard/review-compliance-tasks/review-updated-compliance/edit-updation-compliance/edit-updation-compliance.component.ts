...
@Component({
  selector: 'app-edit-updation-compliance',
  templateUrl: './edit-updation-compliance.component.html',
  styleUrls: ['./edit-updation-compliance.component.scss']
})
export class EditUpdationComplianceComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditUpdationComplianceComponent>,
...
onUpdate() {
  ...
  //formData.append('actual_complied_date', this.complianceData.actual_complied_date || '');
  if (this.complianceData.actual_complied_date) {
    // Append the valid Date object
    const formattedDate = new Date(this.complianceData.actual_complied_date); // Create Date object
    formData.append(`actual_complied_date`, formattedDate.toISOString().split('T')[0]); // Send only date part
  }  else {
  ...
  formData.append('reviewer_remarks', reviewRemarks);
...
console.log(JSON.stringify(formData))
  this.http.put(`${URL}/ReviewCompliance/ReviewerUpdateComplianceDetailsById`, formData, { headers })
    .subscribe(
      (response: any) => {
...
onReject() {
  ...
  this.http.delete(`${URL}/ReviewCompliance/ReviewerRejectComplianceById`, {
