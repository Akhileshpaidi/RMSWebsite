...
@Component({
  selector: 'app-compliance-notified-status',
  templateUrl: './compliance-notified-status.component.html',
  styleUrls: ['./compliance-notified-status.component.scss']
})
export class ComplianceNotifiedStatusComponent {
  dataSource: any;
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    this.dataSource = new CustomStore({
        ...
        load: () => this.sendRequest(URL + '/compliancenotifiedstatus/GetcompliancenotifiedstatusDetails'),
        
        insert: (values) => this.sendRequest(URL + '/compliancenotifiedstatus/InsertcompliancenotifiedstatusDetails', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/compliancenotifiedstatus/UpdatecompliancenotifiedstatusDetails', 'PUT', {
             key,
            //  values: JSON.stringify(values)
            values
         }),
         remove: (key) => this.sendRequest(URL + '/compliancenotifiedstatus/DeletecompliancenotifiedstatusDetails', 'DELETE', {
             key
         })
    });
