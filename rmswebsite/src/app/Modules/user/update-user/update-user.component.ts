...
export class UpdateUserComponent implements OnInit {
  ...
  updateUser(value: any) {
    ...
    let payload = {
     // authid: this.sessionData.profile.authid,
      //mode: 'U',
      USR_ID: this.userData.usR_ID,
      firstname: value.firstname,
      USR_LOGIN: value.username,
      emailid: value.emailid,
      Designation: value.Designation,
      mobilenumber: value.mobilenumber,
      Entity_Master_id:value.entity,
      Unit_location_Master_id:value.Location,
      Department_Master_id:value.Department,
    ...
    //     }
    // );
    this.http.post(URL + '/createuserDetails/UpdatecreateuserDetails', payload, { headers })
  .subscribe({
      next: (response: any) => {
  ...
  selectUser() {
    let id: any = window.history.state.data.usR_ID;
    let user: any = this.session.getUser();
    this.sessionData = JSON.parse(user);
...
//alert(id)
    this.http.get<any[]>(URL + '/createuserDetails/GetIndividualUserData?userid='+id, { headers })
      .subscribe({
        next: (res:any) => {
          res.forEach((element:any) => {
           ...
           // const dobDate: Date = new Date(element.dob);
            this.http.get<any[]>(URL + '/UnitLocationMaster/GetUnitLocationDetails?Entity_Master_id='+element.entity_Master_id, { headers })
            .subscribe({
              next: (res) => {
            ...
            this.getUnitLocation(element.entity_Master_id);
            this.updateUserForm.patchValue({
             ...
             // roles:element.roles.split(",").map(Number),
              ...
              entity:element.entity_Master_id,
              Location:element.unit_location_Master_id,
              Department:element.department_Master_id,
  ...
  openDialog(): void {
    console.log('User ID before opening dialog:', this.userData?.usR_ID);
    let dialogRef = this.dialog.open(UserPassResetComponent, {
      data: {
        callback: this.callBack.bind(this),
        defaultValue: this.userData?.usR_ID,
        userId: this.userData?.usR_ID,
      },
      width: '50%',
    });
    dialogRef.afterClosed().subscribe((result) => {
