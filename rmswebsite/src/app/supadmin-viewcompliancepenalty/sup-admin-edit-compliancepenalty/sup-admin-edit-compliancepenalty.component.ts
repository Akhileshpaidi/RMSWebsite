...
@Component({
  selector: 'app-sup-admin-edit-compliancepenalty',
  templateUrl: './sup-admin-edit-compliancepenalty.component.html',
  styleUrls: ['./sup-admin-edit-compliancepenalty.component.scss']
})
export class SupAdminEditCompliancepenaltyComponent {
  actname: any;
...
ngOnInit(): void {
  ...
  retrieveFiles(compliancespenatlyid: any): void {
    this.http.get(`${BASE_URL}/SupAdmin_penaltycategory/GetpenaltysupcomplianceDetailsnyid/${compliancespenatlyid}`)
    .subscribe((data: any) => {
  ...
  removeReferenceField(index: number,compliance_filepenalty_id:any): void {
    ...
    // If bare_act_id is not 0, make an HTTP POST request to remove the file
     this.http.post(`${BASE_URL}/SupAdmin_penaltycategory/removesuppenaltycategory/${compliance_filepenalty_id}`, {})
      .subscribe(
        (data: any) => {
    ...
    SubmitUpdateForm(): void {
      ...
      if (this.UpdateForm.valid) {
       ...
       // alert('About to submit form data to API. Confirm?');
        ...
        this.http.post(`${BASE_URL}/SupAdmin_penaltycategory/Updatesuppenaltycompliance`, formData, {
       ...
       //   alert('Compliance Penalty Updated  successfully!');
          this.router.navigate(['/dashboard/SuperAd_Common_Entity/supadmin-viewcompliancepenalty']);
        }, error => {
