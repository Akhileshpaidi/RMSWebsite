...
@Component({
  selector: 'app-view-action-taken-mitigation',
  templateUrl: './view-action-taken-mitigation.component.html',
  styleUrls: ['./view-action-taken-mitigation.component.scss']
})
export class ViewActionTakenMitigationComponent {
...
this.dataSource = new CustomStore({
  ...
  key: 'suggestions_id',
  load: () => this.sendRequest(URL + '/MitigationController/GetAssessmentsListManagement/'+this.mitigations_id),
  
  insert: (values) => this.sendRequest(URL + '/MitigationController/insertTasksManagement/'+this.UserIdValue, 'POST', {
...
makeAsyncDataSource(http:HttpClient) {
  ...
  return new CustomStore({
    ...
    key: 'trackerID',
    load() {
      return lastValueFrom(http.get(`${URL}/ViewMitigationController/GetViewAssessmentActionList`, { headers }));
    },
  });
}
...
this.SuggestionsInfo.acknowledge = 1;
...
sendformanagement(){
  ...
  this.http.post(URL+'/MitigationController/UpdateTasksStatus/'+this.TrackerId,{headers}).subscribe((data:any)=>{
    
    console.log('API Response:', data);

    const message = "Changes Sent Successfully";
    const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
      width: '550px',
      data: { message: message},
...
this.dataSource = new CustomStore({
  ...
  key: 'suggestions_id',
  load: () => this.sendRequest(URL + '/ViewMitigationController/GetViewAssessmentActionList/'+this.TrackerId),
...
this.mitigationForm.get('Acknowledge')?.setValue(this.selectedRowData.acknowledge);
this.mitigationForm.get('ActionRequired')?.setValue(this.selectedRowData.action_required);
this.mitigationForm.get('Comments')?.setValue(this.selectedRowData.remarks);
this.mitigationForm.get('ManInputReq')?.setValue(this.selectedRowData.notify_management);
this.mitigationForm.get('EndDate')?.setValue(this.selectedRowData.input_date);
this.mitigationForm.get('SelectPo')?.setValue(this.selectedRowData.assign_responsibility);
this.mitigationForm.get('Tentativedate')?.setValue(this.selectedRowData.tentative_timeline);
this.mitigationForm.get('SuggestiveDoc')?.setValue(this.selectedRowData.suggested_documents);
// Extract File Name from Path
const filePath = this.selectedRowData.file_path;

// this.selectedRowData.fileName = filePath ? filePath.split('/').pop() : 'No File';

// // Construct File URL (Assuming Base URL is Stored)
// const baseUrl = 'https://localhost:5001/Resources/Uploads/';
// this.selectedRowData.fileUrl = filePath ? `${baseUrl}${this.selectedRowData.fileName}` : '';
...
if(this.selectedRowData.status!="completed" ){
  ...
  this.PopUpFields = [
    ...
    },
    {
      ...
      cellTemplate: 'downloadFileTemplate' // Add a custom template for the file
    },
 ...
 this. PopUpFields= [
    ...
    },
    {
      dataField: 'file_path',
      caption: 'Document Evidence',
      cellTemplate: 'downloadFileTemplate' 
    },
    {
      dataField: 'pO_remarks',
  ...
  this.gridColumnsforTasks=this.tempDataSource;
