...
@Component({
  selector: 'app-managementinputs',
  templateUrl: './managementinputs.component.html',
  styleUrls: ['./managementinputs.component.scss']
})
export class ManagementinputsComponent {
  @ViewChild('stepper')
  stepper!: MatStepper;
...
makeAsyncDataSource(http:HttpClient) {
  ...
  return new CustomStore({
    ...
    key: 'trackerID',
    load() {
      return lastValueFrom(http.get(`${URL}/MitigationController/GetMitigationListManagement`, { headers }));
    },
  });
}
...
UpdateAction(){
    ...
    let SuggestionId;
    let SelectPo=this.taskForm.value.SelectPo;
    let Tentativedate=this.taskForm.value.Tentativedate;
    let SuggestiveDoc=this.taskForm.value.SuggestiveDoc;
    let Management_Remarks=this.taskForm.value.Management_Remarks;
     formData = {
      ...
      this.http.post(URL+'/MitigationController/UpdateTasksStatus',formData,{headers}).subscribe((data:any)=>{
    
        console.log('API Response:', data);
    
        const message = "Mitigation Task Assigned Successfully";
        const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
          width: '550px',
          data: { message: message},
...
this.dataSource = new CustomStore({
  ...
  key: 'suggestions_id',
  load: () => this.sendRequest(URL + '/MitigationController/GetAssessmentsListManagement/'+this.TrackerId),
  
  insert: (values) => this.sendRequest(URL + '/MitigationController/insertTasksManagement/'+this.UserIdValue+'/'+this.TrackerId, 'POST', {
      //values: JSON.stringify(values)
      values
  }),
  update: (key, values) => this.sendRequest(URL + '/MitigationController/UpdateTaskListManagement', 'PUT', {
       key,
       //values: JSON.stringify(values)
      values
   }),
