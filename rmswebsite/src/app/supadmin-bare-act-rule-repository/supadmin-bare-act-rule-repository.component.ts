...
@Component({
  selector: 'app-supadmin-bare-act-rule-repository',
  templateUrl: './supadmin-bare-act-rule-repository.component.html',
  styleUrls: ['./supadmin-bare-act-rule-repository.component.scss']
})
export class SupadminBareActRuleRepositoryComponent {
  actname:any;
  rulename:any;
  actid:any;
  SelectedAct: any;
  ...
  act_rule: any;
  actregulatory :any[]=[];
  actrulerepositiory:any;
  isLinkCopied: boolean = false;
  isLinkCopied1: boolean = false;
...
getRules(event: any) {
 ...
 // To get bare act links and filee
 ...
 getactruledata(selectedactrule: any) {
  ...
  this.http.get(URL + '/SupAdmin_ActRegulatory/superadminGetactruleandprocudreDetails/' + this.act_rule_regulatory_id, { headers })
    .subscribe(
      (res: any) => {
        this.act_rule = res;
...
extractFilenameFromUrl1(url: string): string {
  // Extract the filename part from the URL
  const parts = url.split('/');
  return parts[parts.length - 1];
}

downloadact(filePath: string, fileType: string): void {
  const apiUrl = `${URL}/superadminactdownload/superadminactDownLoadFiles?filePath=${filePath}`;
...
//alert (JSON.stringify(res))
    ...
    const filenameFromUrl = this.extractFilenameFromUrl1(filePath);
...
downloadrule(filePath: string, fileType: string): void {
  const apiUrl = `${URL}/SupAdmin_ActRegulatory/superadminactruleDownLoadFiles?filePath=${filePath}`;
