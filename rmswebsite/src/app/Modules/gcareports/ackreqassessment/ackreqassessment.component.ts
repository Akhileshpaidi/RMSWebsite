...
@Component({
  selector: 'app-ackreqassessment',
  templateUrl: './ackreqassessment.component.html',
  styleUrls: ['./ackreqassessment.component.scss']
})
export class AckreqassessmentComponent {
  ...
  constructor(
    ...
    // this.range.get('start')?.setValue(this.oneMonthAgo);
  ...
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsController/getAssScheduledList', formData)
                .then(response => {
    ...
    if (e.format === 'xlsx') {
      ...
      exportDataGrid({ 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListOfAckRequestedAssessment.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Acknowledgement Requested Assessment", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('ListOfAckRequestedAssessment.pdf');
      });
    }
