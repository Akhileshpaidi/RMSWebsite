...
export class UserListComponent implements OnInit,AfterViewInit  {
  ...
  editUser(element: any) {
    ...
    this.router.navigate(['dashboard/user/update-user'], {
      state: { data: element },
    });
  }
  createUser() {
  ...
  getUserList() {
    ...
    this.http.get<any[]>(URL + '/createuserDetails/GetcreateuserDetails', { headers })
    .subscribe({
      next: (res) => {
  ...
  changeStatus(element: any): void {
    ...
    element.usR_STATUS = element.usR_STATUS === 'Active' ? 'Inactive' : 'Active';
    const headers = {
      'Content-Type': 'application/json', // Adjust content type as needed
    };
 // alert(userId);
    // Make the HTTP DELETE request
    this.http.delete(URL + '/createuserDetails/DeletecreateuserDetails?userid='+ element.usR_ID, { headers })
      .subscribe({
        next: (response: any) => {
          const message = `Status changed to ${element.usR_STATUS}`;
          //alert(message);
  ...
  openDialog(userId: string): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) { // User clicked "OK" in the confirmation dialog
        ...
        // Make the HTTP DELETE request
        this.http.delete(URL + '/createuserDetails/DeletecreateDetails?userid=' + userId, { headers })
          .subscribe({
            next: (response: any) => {
