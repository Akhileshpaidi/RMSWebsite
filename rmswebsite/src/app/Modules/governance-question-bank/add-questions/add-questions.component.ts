...
remove(tag: Tag): void {
  ...
  this.chipListInput.nativeElement.value = '';
    ...
    selectedCount='2';
    selectedType=1;
    constructor(private http: HttpClient,private fb: FormBuilder, public dialog: MatDialog,private formBuilder: FormBuilder,private ref: ChangeDetectorRef){
      this.OptionA=true;
      this.OptionB=false;
      this.OptionC=true;
      this.Answers=true;
  ...
  getSubTypes(event: any) {
   ...
   validation(){
    ...
    // alert(correctAnswerControl)
      ...
      //  alert('The correct answer must match the number of selection choices.');
...
if(correctanswer==''||null){
  ...
  return;
    ...
    console.log('Form Data:', formData);
      ...
      this.http.post(URL + '/QuestionBank/AddQuestions',formData)
      .subscribe((response: any) => {
        console.log('Data Save Succefully ', response);
        // Handle the response from the server if needed
        //window.alert('Data Saved Successfully');
        const message = "Data Save Succefully.";
    const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
      width: '550px',
      data: { message: message},
      ...
      let invalidFields:any = Object.keys(this.questionForm.controls)
        ...
        .map(key => {
          switch(key) {
            ...
            case 'no_of_selectionchoices': return 'No of Selection choices ';
    ...
    //   if(this.questionForm.controls["fullresponse"].value==""){

    //     invalidFields.push("correct answer")
    // }
      }
      else{
      //   if(this.questionForm.controls["fullresponse"].value==""){
      ...
      // }
        ...
        // this.questionForm.get('response_type').valueChanges.subscribe((value: string) => {
        //   const textControl = this.questionForm.get('fullresponse'); // Replace 'text_field' with the name of your text field control
        //   if (value === 'text') {
