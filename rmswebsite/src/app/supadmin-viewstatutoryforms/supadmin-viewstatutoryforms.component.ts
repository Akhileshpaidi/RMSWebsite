...
@Component({
  selector: 'app-supadmin-viewstatutoryforms',
  templateUrl: './supadmin-viewstatutoryforms.component.html',
  styleUrls: ['./supadmin-viewstatutoryforms.component.scss']
})
export class SupadminViewstatutoryformsComponent {
  statutoryform:any;
...
getRules1(event: any) {
 ...
 getactruledata(event:any){
   ...
   store: new CustomStore({
       ...
       loadMode: 'raw',
       load:()=>{return new Promise((resolve, reject) => {
         this.http.get(URL + '/SupAdmin_statutoryfrom/GetformsuprecordByID/'+this.ruleid, {headers})
           .subscribe(res => {
            (resolve(res));
...
getRules(selectedfrom: any) {
  ...
  this.http.get(URL + '/SupAdmin_statutoryfrom/GetsupstatutoryDetailsnyid/' + this.statutoryformsid, { headers })
    .subscribe(
      (res: any) => {
        this.act = res;
...
download(filePath: string, fileType: string): void {
  const apiUrl = `${URL}/SupAdmin_statutoryfrom/statutorysupdownloadFiles?filePath=${filePath}`;
...
editAct(statutoryid: number) {
 ...
 // alert(statutoryid)
  ...
  this.router.navigate(['dashboard/SuperAd_Common_Entity/supadmin-editstatutoryforms'], {
