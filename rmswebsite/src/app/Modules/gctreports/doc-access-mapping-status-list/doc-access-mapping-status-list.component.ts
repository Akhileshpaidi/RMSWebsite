...
@Component({
  selector: 'app-doc-access-mapping-status-list',
  templateUrl: './doc-access-mapping-status-list.component.html',
  styleUrls: ['./doc-access-mapping-status-list.component.scss']
})
export class DocAccessMappingStatusListComponent {
...
onRowSelectionChanged(e:any):void{
  ...
  if(selectedRowData.noOfMappedUsers>0){
...
this.DateSearchFilter();
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List of Document Access Mapping Status']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Document Access Mapping Status.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Document Access Mapping Status", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('List of Document Access Mapping Status.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Document Access Mapping Status.csv");
      });
    }); 
    e.cancel = true;
  } 
