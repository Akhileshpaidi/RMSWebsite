...
import { regionmaster} from 'src/app/inspectionservices.service';
   ...
   SectorModelinfo:regionmaster=new regionmaster();
    ...
    constructor(private http: HttpClient,  private session: SessionService ) {
      this.dataSource = new CustomStore({
          ...
          key: 'regionMasterID',
          load: () => this.sendRequest(URL + '/Region/GetRegionDetails'),
          
          insert: (values) => this.sendRequest(URL + '/Region/InsertRegionDetails', 'POST', {
              //values: JSON.stringify(values)
              values
          }),
          update: (key, values) => this.sendRequest(URL + '/Region/UpdateRegionDetails', 'PUT', {
               key,
               //values: JSON.stringify(values)
              values
           }),
           remove: (key) => this.sendRequest(URL + '/Region/DeleteRegionDetails', 'DELETE', {
               key
           })
      });
    }
      ...
      setMissionValue(rowData: any, value: any): void {
        ...
        case 'DELETE':
 // alert(data.key);
  this.http.delete(URL + '/Sub_Region/Sub_RegionDetailsget?id=' + data.key, { headers })
    .subscribe((res: any) => {
         return new Promise((resolve, reject) => {
                this.http.delete(url+'?id='+data.key)
                  .subscribe(res => {
