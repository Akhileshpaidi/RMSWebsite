...
@Component({
  selector: 'app-assess-temp-list',
  templateUrl: './assess-temp-list.component.html',
  styleUrls: ['./assess-temp-list.component.scss']
})
export class AssessTempListComponent {
...
//alert(JSON.stringify(formData))
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsController/GetAllAssesTemplates', formData)
                .then(response => {
    ...
    if (e.format === 'xlsx') {
      ...
      exportDataGrid({ 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListOfAssessmentTemplates.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Assessment Templates", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('ListOfAssessmentTemplates.pdf');
      });
    }
