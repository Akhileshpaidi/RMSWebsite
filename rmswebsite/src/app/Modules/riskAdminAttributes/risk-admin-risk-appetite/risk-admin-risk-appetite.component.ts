...
import { risk_admin_riskappetite, risk_intensity, risk_natureof_cont_occu, riskImpactRating, RiskInherentRatingLevel, RiskLikelihood} from '../../../inspectionservices.service';
...
@Component({
  selector: 'app-risk-admin-risk-appetite',
  templateUrl: './risk-admin-risk-appetite.component.html',
  styleUrls: ['./risk-admin-risk-appetite.component.scss']
})
export class RiskAdminRiskAppetiteComponent {
  dataSource: any;
  start:any=1;
  inputAttr:any;
  userdata:any;
  ...
  max:number=0;
  valArray:number[][]=[];
  riskCatlev:risk_admin_riskappetite=new risk_admin_riskappetite();

  constructor(private http: HttpClient, private session: SessionService,) {
    this.dataSource = new CustomStore({
        key: 'risk_admin_RiskAppetiteId',
        load: () => this.sendRequest(URL + '/risk_admin_riskappetite/Getrisk_admin_riskappetite'),
        
        insert: (values) => this.sendRequest(URL + '/risk_admin_riskappetite/Insertrisk_admin_riskappetite', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/risk_admin_riskappetite/Updaterisk_admin_riskappetite', 'PUT', {
             key,values
         }),
         remove: (key) => this.sendRequest(URL + '/risk_admin_riskappetite/Deleterisk_admin_riskappetite', 'DELETE', {
             key
         })
    });
...
onValueChanged(evt: any, data: any): void {  
 ...
 // alert(JSON.stringify(data))
    this.riskCatlev.risk_admin_RiskAppetiteId=0;
    
     this.params(data);
   }
   
   updateParameters(data:any={}){
   this.riskCatlev.risk_admin_RiskAppetiteId=data.key;
   ...
   // this.riskCatlev.risk_natureof_cont_occu_id=data.values.risk_natureof_cont_occu_id;
    this.riskCatlev.risk_admin_RiskAppetiteName=data.values.risk_admin_RiskAppetiteName;
    this.riskCatlev.risk_admin_RiskAppetiteDesc=data.values.risk_admin_RiskAppetiteDesc;
