...
import { EditUpdationComplianceComponent } from './edit-updation-compliance/edit-updation-compliance.component';
...
@Component({
  selector: 'app-review-updated-compliance',
  templateUrl: './review-updated-compliance.component.html',
  styleUrls: ['./review-updated-compliance.component.scss']
})
export class ReviewUpdatedComplianceComponent implements OnDestroy {
  originalData: any[] = [];
  ...
  constructor(private fb: FormBuilder, private complianceDataService: EComplianceDataService, public dialog: MatDialog, private routers: Router, private router: ActivatedRoute, private http: HttpClient) { 
    ...
    const Userid = parsedData ? parsedData.profile.userid : null;
      ...
      this.Reviewerform = new FormGroup({
        ReviewerRemarks: new FormControl('', [Validators.maxLength(250)]),
      });
  }

  ngOnInit(): void {
    this.originalData = this.complianceDataService.getReviewUpdateComplianceData();
...
onCancel() {
  ...
  this.routers.navigate(['dashboard/e-compliance-dashboard/Review']);
}
ngOnDestroy(): void {
  localStorage.removeItem('RUCcomplianceData');
  // Remove other specific items as needed
}

edit(compliance: any) {
  const dialogRef = this.dialog.open(EditUpdationComplianceComponent, {
    width: '850px',
    data: {
...
handleComplianceUpdate(updatedCompliance: any) {
  ...
  if (updatedCompliance.action === 'updated') {
    ...
    if (index !== -1) {
      ...
      this.complianceDataService.setReviewUpdateComplianceData(this.originalData);
    }
...
onUpdate() {
  const reviewRemarks = this.Reviewerform.controls['ReviewerRemarks'].value;
  ...
  const updatedComplianceData = this.originalData.map(item => ({
    ...
    reviewer_remarks: reviewRemarks,
    updated_by: this.Useridvalue
  }));

  this.http.put(`${URL}/ReviewCompliance/ReviewerBulkUpdateComplianceDetails`, updatedComplianceData)
  .subscribe(
    (response: any) => {
...
onReject() {
  const reviewRemarks = this.Reviewerform.controls['ReviewerRemarks'].value?.trim();
  ...
  this.http.delete(`${URL}/ReviewCompliance/ReviewerRejectBulkCompliance`, {
