...
@Component({
  selector: 'app-ack-requested-doc-list',
  templateUrl: './ack-requested-doc-list.component.html',
  styleUrls: ['./ack-requested-doc-list.component.scss']
})
export class AckRequestedDocListComponent {
...
this.DateSearchFilter();
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List of Published Documents Pending Acknowledgement']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Published Documents Pending Acknowledgement.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Published Documents Pending Acknowledgement", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('List of Published Documents Pending Acknowledgement.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List of Published Documents Pending Acknowledgement.csv");
      });
    }); 
    e.cancel = true;
  } 
