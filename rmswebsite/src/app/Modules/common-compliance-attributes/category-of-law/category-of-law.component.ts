...
@Component({
  selector: 'app-category-of-law',
  templateUrl: './category-of-law.component.html',
  styleUrls: ['./category-of-law.component.scss']
})
export class CategoryOfLawComponent {
  dataSource: any;
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    this.dataSource = new CustomStore({
        ...
        load: () => this.sendRequest(URL + '/catageorylaw/Getcatageorylaws'),
        
        insert: (values) => this.sendRequest(URL + '/catageorylaw/Insertcatageorylaw', 'POST', {
            // values: JSON.stringify(values)
            values
        }),
        update: (key, values) => this.sendRequest(URL + '/catageorylaw/Updatecatageorylaw', 'PUT', {
             key,
            //  values: JSON.stringify(values)
            values
         }),
         remove: (key) => this.sendRequest(URL + '/catageorylaw/Deletecatageorylaw', 'DELETE', {
             key
         })
    });
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['Category of Law']);
      worksheet.addRow([]); 
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "Category of Law.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text('Category of Law  ',75,10);
      doc.setFontSize(12);
