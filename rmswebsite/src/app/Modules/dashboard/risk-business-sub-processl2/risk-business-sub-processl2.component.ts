import { BusinessSubProcessL1, BusinessSubProcessL2 } from 'src/app/inspectionservices.service';
...
@Component({
  selector: 'app-risk-business-sub-processl2',
  templateUrl: './risk-business-sub-processl2.component.html',
  styleUrls: ['./risk-business-sub-processl2.component.scss']
})
export class RiskBusinessSubProcessl2Component {
  ...
  Department:any;
  Function:any;
  Buinessinfo:BusinessSubProcessL2=new BusinessSubProcessL2();
  myService: any;
  ...
  constructor(private http: HttpClient, private session: SessionService) {
    this.dataSource = new CustomStore({
      ...
      load: () => this.sendRequest1(URL + '/BusinessSubProcessL2/GetBusinessSubProcesssL2'),
      
      insert: (values) => this.sendRequest1(URL + '/BusinessSubProcessL2/InsertBusinessSubProcessL2', 'POST', {
          // values: JSON.stringify(values)
          values
      }),
      update: (key, values) => this.sendRequest1(URL + '/BusinessSubProcessL2/UpdateBusinessSubProcessL2', 'PUT', {
           key,
          //  values: JSON.stringify(values)
          values
       }),
       remove: (key) => this.sendRequest1(URL + '/BusinessSubProcessL2/DeleteBusinessSubProcessL2', 'DELETE', {
           key
       })
  ...
  setobjective(this: Column, newData: any, value: number, currentRowData: any) {
    newData.subProcessObjestive = null;
    if (this.defaultSetCellValue !== undefined) {
      this.defaultSetCellValue(newData, value, currentRowData);
    } else {
            console.error('defaultSetCellValue is undefined');
  ...
  onInitNewRow(e: any): void {
    e.data.subProcessObjestive = 'Loading...'; // Temporary default
    this.http.get<{ subProcessObjestive: string }>(URL + '/BusinessSubProcessL1/GetBusinessSubProcesssL1', { headers })
        .subscribe(
            (res: any) => {
              e.data.subProcessObjestive = res.subProcessObjestive || res.data?.subProcessObjestive || 'Default Objective Text';

                console.log('Fetched subProcessObjestive:', res.subProcessObjestive);
            ...
            (err) => {
                console.error('Error fetching subProcessObjestive:', err);
            }
        );
}
...
getunitlocation(event: any) {
  ...
  insertParameters(data:any={}){
    ...
    this.Buinessinfo.businessProcessL2ID=0;
    this.params(data);
   }
   
   updateParameters(data:any={}){
   this.Buinessinfo.businessProcessL2ID=data.key;
   ...
   params(data:any={}){
    ...
    this.Buinessinfo.businessProcessL1ID =data.values.businessProcessL1ID;
    this.Buinessinfo.businessSubProcessL2Name =data.values.businessSubProcessL2Name;
     this.Buinessinfo.businessSubProcessL2Description =data.values.businessSubProcessL2Description;
     this.Buinessinfo.subProcessObjestiveL2 =data.values.subProcessObjestiveL2;
