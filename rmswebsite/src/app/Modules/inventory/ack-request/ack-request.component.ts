...
import { AfterViewInit, ChangeDetectorRef,EventEmitter,OnChanges,ViewChild} from '@angular/core';
...
export class AckRequestComponent implements OnInit,AfterViewInit, OnChanges {
[x: string]: any;
panelOpenState = true;
      ...
      load: () => {
        return new Promise((resolve, reject) => {
         ...
         //console.log('USR_ID',userId)
          this.http.get(URL + '/Acknowledgement/GetAcknowlwdgedata?USR_ID='+userId, {
            //this.http.get(URL + '/Acknowlwdgedata/GetAcknowlwdgedataDetails?USR_ID='+userId, {
            headers,
            params: params
          }).subscribe(
            res => {
...
formatDate(dateString: string | null | undefined): string {
  ...
  return `${day}/${month}/${year}`; // Return in "YYYY/Mon/DD" format
   ...
   updateStatus(data:any) {
    ...
    let payload ={
     USR_ID:this.userdata.profile.userid,
     AddDoc_id :JSON.parse(this.selectedValue)
    }
    
    //alert(JSON.stringify(payload))
    this.http.put(URL+ '/Acknowledgement/UpdateDocStatus/', payload)
...
e(event: any) {
 ...
 // pdfSrc = 'https://files.fm/down.php?i=axwasezb&n=SSaD.docx';
  ...
  onPageChange(event: any): void {
    ...
    } else if (event instanceof AckRequestComponent) {
      // Use type assertion to inform TypeScript about the specific type
      page = (event as AckRequestComponent).getCurrentPage();
    } else {
