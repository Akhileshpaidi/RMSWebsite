...
@Component({
  selector: 'app-crc-mitigation-action-plan',
  templateUrl: './crc-mitigation-action-plan.component.html',
  styleUrls: ['./crc-mitigation-action-plan.component.scss']
})
export class CrcMitigationActionPlanComponent {
range = new FormGroup({
...
let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsContoller/MitigationActionPlancrc', formData)
                .then(response => {
...
this.dataSource = new CustomStore({
    ...
    load: () => {
        ...
        return this.sendRequest1(URL + '/GovControlReportsContoller/MitigationTaskListcrc/'+mitId,formData)
            .then(response => {
                console.log("Request successful, response: ", response);
                return response;
            })
