...
import { risk_admin_residual_risk_rating} from 'src/app/inspectionservices.service';
...
@Component({
  selector: 'app-risk-admin-residu-risk-rat',
  templateUrl: './risk-admin-residu-risk-rat.component.html',
  styleUrls: ['./risk-admin-residu-risk-rat.component.scss']
})
export class RiskAdminResiduRiskRatComponent {
  dataSource:any;
  riskCate:risk_admin_residual_risk_rating=new risk_admin_residual_risk_rating();
  min:number=0;
  max:number=0;
  userdata:any;
  valArray:number[][]=[];
  ...
  constructor(private http: HttpClient,public dialog: MatDialog, private session: SessionService,){
    ...
    this.dataSource = new CustomStore({
      ...
      load: () => this.sendRequest(URL + '/RiskSupAdminController/Getrisk_admin_residual_risk_ratings'),
      
      insert: (values) => this.sendRequest(URL + '/RiskSupAdminController/Insertrisk_admin_residual_risk_ratings', 'POST', {
          // values: JSON.stringify(values)
          values
      }),
      update: (key, values) => this.sendRequest(URL + '/RiskSupAdminController/Updaterisk_admin_residual_risk_ratings', 'PUT', {
           key,values
       }),
       remove: (key) => this.sendRequest(URL + '/RiskSupAdminController/Deleterisk_admin_residual_risk_ratings', 'DELETE', {
           key
       })
  });
