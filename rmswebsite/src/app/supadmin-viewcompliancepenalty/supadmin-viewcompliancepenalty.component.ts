...
@Component({
  selector: 'app-supadmin-viewcompliancepenalty',
  templateUrl: './supadmin-viewcompliancepenalty.component.html',
  styleUrls: ['./supadmin-viewcompliancepenalty.component.scss']
})
export class SupadminViewcompliancepenaltyComponent {
  ...
  //alert(this.actid)
   ...
   this.compliancepenalty={
      ...
      store: new CustomStore({
          ...
          loadMode: 'raw',
          load:()=>{return new Promise((resolve, reject) => {
            this.http.get(URL + '/SupAdmin_penaltycategory/GetpenaltysupcomplianceByID/'+this.ruleid, {headers})
              .subscribe(res => {
               (resolve(res));
...
getRules(selectedfrom: any) {
  ...
  this.http.get(URL + '/SupAdmin_penaltycategory/GetpenaltysupcomplianceDetailsnyid/' + this.compliancepenaltyid, { headers })
    .subscribe(
      (res: any) => {
        this.act = res;
...
download(filePath: string, fileType: string): void {
  const apiUrl = `${URL}/SupAdmin_penaltycategory/compliancesuppenaltydownloadFiles?filePath=${filePath}`;
...
editAct(penaltyid: number) {
 ...
 // alert(penaltyid)
  ...
  this.router.navigate(['dashboard/SuperAd_Common_Entity/supAdmin-editcompliancepenalty'], {
