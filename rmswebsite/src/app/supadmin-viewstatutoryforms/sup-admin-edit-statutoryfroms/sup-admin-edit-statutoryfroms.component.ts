...
@Component({
  selector: 'app-sup-admin-edit-statutoryfroms',
  templateUrl: './sup-admin-edit-statutoryfroms.component.html',
  styleUrls: ['./sup-admin-edit-statutoryfroms.component.scss']
})
export class SupAdminEditStatutoryfromsComponent {
...
retrieveFiles(statutoryformsid: any): void {
  this.http.get(`${BASE_URL}/SupAdmin_statutoryfrom/GetsupstatutoryDetailsnyid/${statutoryformsid}`)
    .subscribe((data: any) => {
...
SubmitUpdateForm(): void {
  ...
  //  alert(JSON.stringify(this.UpdateForm.value));
   ...
   console.log(formData)
    ...
    this.http.post(`${BASE_URL}/SupAdmin_statutoryfrom/Updatesupstatutory`, formData, {
    ...
    .subscribe(response => {
      this.replacedFiles.forEach((statutory_forms_filemaster_id:any)=>{
        this.http.post(`${BASE_URL}/SupAdmin_statutoryfrom/remosupvestatutoryfrom/${statutory_forms_filemaster_id}`, {})
        .subscribe(
          (data: any) => {
            console.log('File removed successfully:', data);
          },
      ...
      this.erroMessage ="Statutory Forms/Records Updated Successfully";
      const dialogRef = this.dialog.open(DaidailogeComponent, {
        width: '400px',
        data: { message: this.erroMessage }
      });
    //  alert('Act Updated  successfully!');
      this.router.navigate(['/dashboard/SuperAd_Common_Entity/supadmin-viewstatutoryforms']);
    }, error => {
