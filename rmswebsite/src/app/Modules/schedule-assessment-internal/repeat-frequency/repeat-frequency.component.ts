...
import { ChangeDetectorRef, NgModule, NgZone, Pipe, PipeTransform } from '@angular/core';
...
@Component({
  selector: 'app-repeat-frequency',
  templateUrl: './repeat-frequency.component.html',
  styleUrls: ['./repeat-frequency.component.scss'],
  template: `
...
export class RepeatFrequencyComponent {
 ...
 repeatMinDate: string | undefined;
   ...
   selectedRowData:any;
      ...
      usergridColumns: any = ['name'];
      usergridColumns1:any=['firstname']
...
containerColumnsNew: any[] = [
  ...
  {
    dataField: 'mapped_users',
    caption: 'Mapped Users'
  },
  {
    dataField: 'type_Name',
    caption: 'Assessment Type'
...
getScheduleAssessmentId(dataSource:any, schedule_Assessment_id:any) {
  ...
  return result ? result.schedule_Assessment_id : 'Not Found';
      ...
      getSubCatDoc():void{
        ...
        this.http.get<any[]>(`${URL}/DocSubCategory/GetDocSubCategoryModelDetailsbyIdnewforarray?${queryParams}`, { headers })
        .subscribe((res: any[]) => {
          console.log('Topic Data API Response:', res);
     ...
     if (this.AssessmentForm.valid) {
      ...
      if(this.repeatFreq===true){
        if(this.AssessmentForm.value.value_Frequency!=0 && this.AssessmentForm.value.repeatEndDate!=null){
          ...
          let repeatEndDate=this.AssessmentForm.value.repeatEndDate;
    ...
    const SubCatDoc: string = this.AssessmentForm.value.DocumentSubCategory?.doc_SubCategoryID?.toString() || '';
       ...
       const formData = {
        ...
        endDate:this.AssessmentForm.value.endDate,
         ...
         frequency_period:this.AssessmentForm.value.frequency_period,
       ...
       //  alert(JSON.stringify(formData));
        ...
        this.http.post(URL + '/SetAssessment/insertRepeatFrequency',formData)
        ...
        .subscribe((response: any) => {
          ...
          const message = "Internal Repeat Frequency Data Saved Successfully.";
          const dialogErrorRef = this.dialog.open(DaidailogeComponent, {
            width: '550px',
            data: { message: message},
 ...
 //const SubCatDoc: string = this.AssessmentForm.value.DocumentSubCategory?.doc_SubCategoryID?.toString() || '';
//const SubCatDoc: number = +this.AssessmentForm.value.DocumentSubCategory || 0;
      ...
      const formData = {
       ...
       this.http.post(URL + '/SetAssessment/insertOnetimeFrequency',formData).subscribe((response: any) => {
  ...
  subDocTypesChanged(e:any){
    //this.doc_SubCategoryID=e.value;
  ...
  //  // alert(JSON.stringify(this.Doc_CategoryID))
...
getsubDocTypes(event: any) {
  this.Doc_CategoryID = event.value;
  this.http.get<any[]>(URL + '/DocSubCategory/GetDocSubCategoryModelDetailsbyId/' + this.Doc_CategoryID, { headers })
    .subscribe(
      (res) => {
        this.Documentsubcategory = res; // <-- Directly assign array
      },
      (err) => {
    ...
    if (e.format === 'xlsx') {
      ...
      exportDataGrid({ 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "ListOfScheduledAssessments.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Scheduled Assessments", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('ListOfScheduledAssessments.pdf');
      });
    }
