...
@Component({
  selector: 'app-superadmin-edit-actregulatory',
  templateUrl: './superadmin-edit-actregulatory.component.html',
  styleUrls: ['./superadmin-edit-actregulatory.component.scss']
})
export class SuperadminEditActregulatoryComponent {
  actregulatoryid:any;
...
retrieveFiles(actregulatoryid: any): void {
  this.http.get(`${BASE_URL}/superadminactregulations/superadminGetactregulationsDetailsnyid/${actregulatoryid}`)
    .subscribe((data: any) => {
...
removeReferenceField(index: number,bare_act_id:any): void {
  ...
  if (bare_act_id !== 0) {
  this.http.post(`${BASE_URL}/superadminactregulations/removesupActregulatoryDetails/${bare_act_id}`, {})
  .subscribe(
    (data: any) => {
...
SubmitUpdateForm(): void {
 ...
 //  alert('About to submit form data to API. Confirm?' + JSON.stringify(  formData));
   console.log(formData)
    ...
    this.http.post(`${BASE_URL}/superadminactregulations/superadminUpdateActRegulatory`, formData, {
    ...
    .subscribe(response => {
      ...
      this.erroMessage ="Act Updated  Successfully";
      const dialogRef = this.dialog.open(DaidailogeComponent, {
        width: '400px',
        data: { message: this.erroMessage }
      });
      //alert('Act Updated  successfully!');
      this.router.navigate(['/dashboard/SuperAd_Common_Entity/supadmin-view-actregulator']);
    }, error => {
