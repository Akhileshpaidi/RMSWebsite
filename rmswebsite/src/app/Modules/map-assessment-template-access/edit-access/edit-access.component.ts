...
export class EditAccessComponent {
  ...
  selectedRowKeys: any[] = []; // always an array for dx-data-grid
...
Userid1:any;
erroMessage:any;
 //  gridColumns: any = ['assessement_ProvideAccessID', 'assessementTemplateID','assessment_name', 'entity_Master_Name','unit_location_Master_name','total_questions','total_estimated_time'];
  // gridColumns: any = ['assessement_ProvideAccessID','assessementTemplateID','assessment_name',
  //   'entity_Master_Name','unit_location_Master_name',{
  ...
  //       return "";
  //     }
  //   },'firstname','total_questions','total_estimated_time'];
    
// usergridColumns: any[] = ['userID','firstname'];
gridColumns: any = [
...
usergridColumns: any[] = ['userloactionmappingID','firstname','entity_Master_Name','unit_location_Master_name'];
 ...
 //   alert(id)
  ...
  gridBox_displayEpxr(user_id:any){
    return user_id && `${user_id.firstname}`;
  }
  gridBox_displayEpxr1(userID:any){
  ...
  onSelectionChanged(selectedItems: any[]) {
    ...
    //this.selectedOption = selectedItems.map(item => item.ID);
  ...
  onGridBoxOptionChanged(e:any) {
    ...
    //               this.http.get(URL + '/Assessment/GetAssUsersByAssTemp?AssessementTemplateID=' + this.assessementtempid, { headers })
    //                 .subscribe(res => {
    //                   (resolve(res));
    //                 }, (err) => {
    //                   reject(err);
...
this.dropdownOptions.load().then((data: any) => {    
  ...
  this.dropdownOptions.load().then((data: any) => {    
    ...
    onuserchange(e:any){
      if (e.name === 'value') {
        this.Userid1= e.value.join('');

    
      this.http.get(URL + '/Assessment/GetAssoneUserRightsPermission?USR_ID='+this.Userid1+"&&AssessementTemplateID="+ this.assessementtempid, {headers})
      .subscribe((response: any) => {
...
// 
      ...
      return new CustomStore({
        ...
        load: () => {
         ...
         return lastValueFrom(
          http.get(url, { headers }).pipe(
            map((res: any) => {
              // Filter out the user with the specific ID
              return res.filter((user: any) => user.usR_ID !== this.userid);
            })
          )
        );
      },
  ...
  ProvideAccessUpdate(){
    ...
    if (invalidFields.length > 0) {
     ...
     if (this.Ass_editProvideAccess_form.valid) {

      const apiUrl = `${URL}/Assessment/GetAssessementProvideAccessByID?ass_provideaccessid=${this.gridBoxValue}`;
      ...
      .subscribe(
        (response: any[]) => {
          ...
          if (response && response.length > 0) {
            const AddDoc_id = response[0].addDoc_id;
            const Assessement_ProvideAccessID = response[0].assessement_ProvideAccessID; // Extract value here

            console.log('AddDoc_id:', AddDoc_id);
            console.log('Assessement_ProvideAccessID:', Assessement_ProvideAccessID);
   ...
   //   alert(JSON.stringify(this.Ass_editProvideAccess_form.value))
    ...
    //  alert(JSON.stringify(formData));
      ...
      this.http.post(URL + '/Assessment/UpdateAssProvideAccess',formData)
      .subscribe((response: any) => {
    ...
    (error: any) => {
      console.error('Error fetching AssessementProvideAccessID:', error);
    }
  );  }
}
}
