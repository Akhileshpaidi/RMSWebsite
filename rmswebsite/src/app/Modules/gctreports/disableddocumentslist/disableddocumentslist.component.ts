...
@Component({
  selector: 'app-disableddocumentslist',
  templateUrl: './disableddocumentslist.component.html',
  styleUrls: ['./disableddocumentslist.component.scss']
})
export class DisableddocumentslistComponent {
...
DateSearchFilter() {
  ...
  let adjustDate = (date: Date | null | undefined) => date ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().split('T')[0] : null;
    ...
    this.dataSource1 = new CustomStore({
        ...
        load: () => {
            ...
            return this.sendRequest1(URL + '/GovControlReportsController/GetDocumentDisabled', formData)
                .then(response => {
  ...
  exportGrid(e:any) {
    if (e.format === 'xlsx') {
      ...
      worksheet.addRow(['List of Disabled Published Documents']);
      worksheet.addRow([]);
      exportDataGrid({ 
        worksheet: worksheet, 
        ...
        workbook.xlsx.writeBuffer().then(function(buffer) { 
          saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List Of Published Documents Disable.xlsx"); 
        }); 
      }); 
      e.cancel = true;
    } 
      ...
      const doc = new jsPDF();
      doc.text("List of Disabled Published Documents", 80,10); // Adjust the position as needed
      doc.setFontSize(12);
      exportDataGridToPdf({
        jsPDFDocument: doc,
        component: e.component,
      }).then(() => {
        doc.save('List Of Published Documents Disable.pdf');
      });
    }
    ...
    exportDataGrid({ 
      ...
      workbook.csv.writeBuffer().then(function(buffer) {
        saveAs(new Blob([buffer], { type: "application/octet-stream" }), "List Of Published Documents Disable.csv");
      });
    }); 
    e.cancel = true;
  } 
